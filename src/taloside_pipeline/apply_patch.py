...
# Replace mol_to_pdbqt_string function body
...
# Find function start and end (end marked by 'return \"\".join(lines)' within that function)
start_idx = text.index(start_sig)
# find the next occurrence of "\n\ndef " after start_idx to locate function end (fallback)
next_def = text.find("\n\ndef ", start_idx)
if next_def == -1:
    # fallback: find the 'def embed_ligand_3d' which follows in original file
...
# Replace Meeko check in validate_receptor
old_block = (
...
if old_block in new_text:
    new_text = new_text.replace(old_block,
        "lactose_pdbqt = lig_dir / \"lactose_redock.pdbqt\"\n\n"
        "        # Use the manual PDBQT writer (do not require Meeko)\n"
        "        lactose_pdbqt.write_text(\n"
        "            mol_to_pdbqt_string(embedded, conf_id=0, name=\"lactose\"),\n"
        "            encoding=\"utf-8\",\n"
        "        )\n"
    ...
    # try a more tolerant replacement: replace the mol_to_pdbqt_string_meeko call if present
