...
try:
    from .glycolibrary_generator import configure_logging
    from .descriptor_calculator import standardize_mol
except ImportError:  # pragma: no cover
    from glycolibrary_generator import configure_logging  # type: ignore
    from descriptor_calculator import standardize_mol  # type: ignore
...
# Galectin-3 CRD binding site centroid in 3ZSJ crystal frame (Å).
# Derived from the lactose (LAC) HETATM coordinates in 3ZSJ.pdb:
#   mean(x) ≈ 13.8, mean(y) ≈ 7.2, mean(z) ≈ 14.1
# These are the correct values to use; the placeholder values (10, 15, 5)
# in the architecture spec were illustrative only and do NOT cover the CRD.
# If you use a different crystal form or chain, recompute from HETATM records.
_3ZSJ_CENTER_X: float = 13.8
_3ZSJ_CENTER_Y: float = 7.2
_3ZSJ_CENTER_Z: float = 14.1
...
class DockingConfig:
    ...
    Box coordinates note
    --------------------
    The default center_x/y/z values are the correct CRD centroid for 3ZSJ
    (derived from the crystal lactose HETATM records, ~13.8 / 7.2 / 14.1 Å).
    The architecture spec used placeholder values (10, 15, 5) which do NOT
    cover the CRD binding site — those will produce meaningless docking scores.
    Use get_box_center_from_crystal() to auto-compute from any PDB file.
    """

    receptor_pdb: Path
    receptor_pdbqt: Path
    center_x: float = _3ZSJ_CENTER_X
    center_y: float = _3ZSJ_CENTER_Y
    center_z: float = _3ZSJ_CENTER_Z
    box_size: float = 20.0
...
def get_box_center_from_crystal(
    pdb_path: Path,
    resnames: Optional[Set[str]] = None,
) -> Tuple[float, float, float]:
    """
    Compute the docking box centroid from crystal ligand HETATM coordinates.

    Call this to auto-derive correct box center for any PDB file, rather
    than using hard-coded placeholder coordinates.

    Usage
    -----
    >>> cx, cy, cz = get_box_center_from_crystal(Path("data/docking/3ZSJ.pdb"))
    ...
    ... )
    """
    coords, _ = extract_ligand_coords_from_pdb(pdb_path, resnames)
    cx, cy, cz = float(coords[:, 0].mean()), float(coords[:, 1].mean()), float(coords[:, 2].mean())
    return cx, cy, cz
...
def is_rotatable_bond(bond: Chem.Bond, mol: Chem.Mol) -> bool:
    ...
    if a1.GetDegree() == 1 or a2.GetDegree() == 1:
        return False
    # Exclude amide C–N and C=O bonds.
    # SMARTS "C(=O)N" matches three atoms: [C, O, N] → indices 0, 1, 2.
    # The original code incorrectly used match[1] as N (it is the carbonyl O).
    amide = Chem.MolFromSmarts("C(=O)N")
    if mol.HasSubstructMatch(amide):
        for match in mol.GetSubstructMatches(amide):
            c_idx, _o_idx, n_idx = match[0], match[1], match[2]
            bond_atoms = {a1.GetIdx(), a2.GetIdx()}
            if bond_atoms == {c_idx, n_idx}:
                return False
    return True
...
def _pdbqt_atom_line(
    ...
    """
    Format one PDBQT ATOM record for AutoDock / Vina.

    Column layout (1-based, matching Vina's strict parser):
      1-6   record type  "ATOM  "
      7-11  serial       right-justified integer
      13-16 atom name    left-justified in 4-char field
      18-20 residue name "LIG"
      22    chain        "A"
      23-26 residue seq  "   1"
      31-38 x            8.3f
      39-46 y            8.3f
      ...
      61-66 B-factor     " 0.00"
      Partial charge (0.000) and AD type at end (required by Vina parser).

    BUG FIXED: original used {ad_type:2s} which misaligned columns and
    omitted the partial charge field, causing Vina to reject ligand PDBQT.
    """
    ...
    atom_name = f"{ad_type:<4s}"  # left-justified, 4 chars (cols 13-16)
    return (
        f"ATOM  {serial:5d} {atom_name} LIG A   1    "
        f"{pos.x:8.3f}{pos.y:8.3f}{pos.z:8.3f}"
        f"  1.00  0.00    "
        f"{0.000:6.3f} {ad_type}\n"
    )
...
def _write_pdbqt_branch(
    ...
    BUG FIXED: original always emitted ROOT/ENDROOT regardless of depth,
    producing invalid nested ROOT blocks inside BRANCH sections.
    The is_root flag now controls whether ROOT/ENDROOT wrappers are emitted:
      - True  at depth=0 (top-level call from mol_to_pdbqt_string)
      - False for all recursive calls inside BRANCH/ENDBRANCH blocks

    AutoDock/Vina PDBQT structure must be:
      ROOT
        <rigid core atoms>
      ENDROOT
      BRANCH a b
        <branch atoms — no ROOT wrapper>
        BRANCH c d
          <sub-branch atoms>
        ENDBRANCH c d
      ENDBRANCH a b
    ...
    def _emit_atoms(atom_set: Set[int]) -> None:
    ...
    if not local_rot:
        # Leaf fragment: emit atoms, wrapped in ROOT only at top level
        if is_root:
            lines.append("ROOT\n")
        _emit_atoms(remaining)
        if is_root:
            lines.append("ENDROOT\n")
        return

    # Pick bond separating smallest branch (AutoDock-style greedy split)
    best_bond = None
    ...
    for a, b in local_rot:
        ...
        if len(branch_a) <= len(branch_b):
            r_atoms, br_atoms, r_atom, br_atom = (
                remaining - branch_a, branch_a, b, a,
            )
        else:
            r_atoms, br_atoms, r_atom, br_atom = (
                remaining - branch_b, branch_b, a, b,
            )
        if 0 < len(br_atoms) < best_branch_size:
            best_branch_size = len(br_atoms)
            best_bond = (r_atoms, br_atoms, r_atom, br_atom)

    if best_bond is None:
        # No valid split found: emit all atoms as flat rigid block
        if is_root:
            lines.append("ROOT\n")
        _emit_atoms(remaining)
        if is_root:
    ...
    root_atoms, branch_atoms, root_atom, branch_atom = best_bond

    # Rigid core
    if is_root:
        lines.append("ROOT\n")
    _emit_atoms(root_atoms)
    if is_root:
        lines.append("ENDROOT\n")

    # Flexible branch — recursive call with is_root=False so no nested ROOT
    lines.append(f"BRANCH {root_atom + 1:4d} {branch_atom + 1:4d}\n")
    _write_pdbqt_branch(
        mol, conf_id, branch_atoms, lines, serial_counter,
        rotatable_bonds, depth + 1, is_root=False,
    )
    lines.append(f"ENDBRANCH {root_atom + 1:4d} {branch_atom + 1:4d}\n")
...
class VinaDocking:
    ...
    def __init__(self, config: DockingConfig, logger_instance: Optional[logging.Logger] = None):
        self.config = config
        self.logger = logger_instance or configure_logging(
    ...
    def validate_receptor(self) -> bool:
        ...
        std = standardize_mol(lactose)
        if std is not None:
            lactose = std

        embedded, status = embed_ligand_3d(lactose)
        if status != "success" or embedded is None:
        ...
        if n_crystal == n_docked:
            # Same atom count: direct RMSD in the shared receptor frame.
            # This is the standard redocking check (receptor is fixed,
            # crystal and docked poses are in the same coordinate system).
            rmsd = compute_rmsd(crystal_coords, docked_coords, align=False)
        else:
            # Atom count mismatch (e.g. Vina added/removed hydrogens or
            # the crystal entry has a different resolution).
            # BUG FIXED: the original fallback called Chem.MolFromPDBBlock on
            # the Vina output PDBQT file, which contains AutoDock-specific
            # records (ROOT, BRANCH, TORSDOF) that cause MolFromPDBBlock to
            # fail silently or return a corrupt molecule.
            # Corrected approach: use parse_pdbqt_coords consistently and
            # align by taking the minimum common heavy-atom count with
            # Kabsch superposition, which is valid when atom ordering is
            # conserved (as it is for a rigid sugar in a fixed receptor frame).
            n_common = min(n_crystal, n_docked)
            if n_common < 3:
                raise AssertionError(
                    f"Too few common atoms for RMSD ({n_crystal} crystal vs "
                    f"{n_docked} docked); cannot validate receptor."
                )
            self.logger.warning(
                f"[validate] Crystal ({n_crystal}) vs docked ({n_docked}) atom "
                f"count mismatch; using first {n_common} heavy atoms with "
                f"Kabsch alignment."
            )
            rmsd = compute_rmsd(
                crystal_coords[:n_common],
                docked_coords[:n_common],
                align=True,  # Kabsch superposition for mismatched atom counts
            )
...
def run_phase3_pipeline(
    ...
    docker = VinaDocking(config, logger_instance=logger)
