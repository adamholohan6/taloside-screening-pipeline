...
def main():
    """
    Demonstration: Generate a small taloside derivative library.
    
    This example shows:
      1. Triazole ring installation at position on the taloside scaffold
      2. Use of ReactionSMARTS.TRIAZOLE_FORMATION pattern
      3. Multiple building blocks (terminal alkynes) varying in electronics
      4. Full lineage tracking and validation
    """
    
    # Core taloside scaffold (simplified parent structure)
    # This is the core carbohydrate + triazole connector framework
    SCAFFOLD_SMILES = (
        "O=C(O[C@H]1[C@@H](OCN2C=CN=N2)[C@@H](O)[C@@H](CO)O[C@H]1OC)C"
    )
    
    # Building block library: aromatic alkynes with varied electronics
    # These represent terminal alkynes we'll couple via click chemistry
    BUILDING_BLOCKS = [
        {
            'id': 'BB-001-Phenyl',
            'smiles': 'C#Cc1ccccc1'  # Phenylacetylene (neutral)
        },
        {
            'id': 'BB-002-4OMe',
            'smiles': 'C#Cc1ccc(OC)cc1'  # 4-Methoxyphenylacetylene (electron-donating)
        },
        {
            'id': 'BB-003-4Cl',
            'smiles': 'C#Cc1ccc(Cl)cc1'  # 4-Chlorophenylacetylene (electron-withdrawing)
        },
        {
            'id': 'BB-004-3Br',
            'smiles': 'C#Cc1cc(Br)ccc1'  # 3-Bromophenylacetylene (moderate EWG)
        },
        {
            'id': 'BB-005-2Pyridine',
            'smiles': 'C#Cc1ccccn1'  # 2-Ethynylpyridine (heteroaryl)
        },
    ]
    ...
    config = LibraryConfig(
        ...
        max_product_mw=900.0,
        include_stereoisomers=True,
        filter_hypervalent=True,
        output_dir=Path("taloside_library")
    )
    ...
    try:
        ...
        if not library_df.empty:
            generator.export_library(library_df, "taloside_derivatives.csv")
            generator.export_failed_products("taloside_failed.csv")
            ...
            print(f"\nDescriptor Summary:")
            print(library_df[['molecular_weight', 'h_donors', 'h_acceptors', 
                              'logp', 'tpsa']].describe())
        else:
            logger.error("Library generation produced no valid products.")
            sys.exit(1)
